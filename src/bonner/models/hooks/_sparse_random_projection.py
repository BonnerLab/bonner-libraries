...
def create_sparse_projection_matrix(
    *,
    ...
    locations = np.stack(np.unravel_index(indices=indices, shape=(n_features, n_components)))

    projection = torch.sparse_coo_tensor(
        indices=torch.from_numpy(locations),
        values=scale * (2 * rng.binomial(n=1, p=0.5, size=n_nonzero) - 1).astype(np.float32),
